...
class Application extends Connection{
    ...
    function select_all_applications($cust_id, $ip){
        return $this ->fetch("select app_id, cust_id, uni_id, ip_add, price, university_name, mission, university_country, university_city, SUM(price) as total from application as a inner join university as u on a.uni_id = u.university_id where cust_id ='$cust_id' or ip_add = '$ip'");
    }

    function select_all_applications_without_customer_id($ip){
        return $this ->fetch("select app_id, cust_id, uni_id, ip_add, price, university_name, mission, university_country, university_city, SUM(price) as total from application as a inner join university as u on a.uni_id = u.university_id where ip_add = '$ip'");
    }
